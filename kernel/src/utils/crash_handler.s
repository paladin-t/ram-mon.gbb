...
; Adapted by Toxa
        ...
        ; .globl    _script_on_error
        ; .globl    _vm_error_handler_bank
...
___HandleCrash::
        push    hl
        ld      hl, #rROMB0
        ; ld      a, (_vm_error_handler_bank)
        ; or      a
        ; jr      nz, 1$                  ; Jump if has custom error handler
        ld      (hl), #b___HandleCrash_banked
        pop     hl
        jp      ___HandleCrash_banked   ; Call the default error handler
1$:
        ; ld      (hl), #1                ; The bank of the custom error handler
        ; pop     hl
        ; jp      _script_on_error        ; Call the custom error handler


        .area   _CODE_1
.globl b___HandleCrash_banked
b___HandleCrash_banked = 1
